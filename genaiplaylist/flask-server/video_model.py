...
# This method searches YouTube for the parameter search query and returns
...
def playlist():
    playlist_links = set()  # Using a set to avoid duplicate links
    # Generate the playlist links by calling get_link for each item from llm_response
    for i in llm_response.lst:
        playlist_links.add(get_link(i))  # add() ensures no duplicates
